...
### Session 2026-02-26
- Q: 关卡数据结构选择？ → A: 使用 Tiled 风格 JSON（正交网格，图层包含地块、碰撞、实体、LOGO）。
- Q: 物理时间步策略？ → A: 固定时间步累积器，60Hz 更新，渲染解耦可插值。
...
- **FR-002**: 角色运动使用固定时间步累积器（≈60Hz）更新重力、加速度与跳跃，渲染解耦且可插值；体验在60fps下流畅，输入响应不超过100ms。
